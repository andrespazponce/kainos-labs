...
// Postea un mensaje de chatter replicando EXACTAMENTE lo que hace el portal
// web nativo de Odoo: el endpoint /mail/message/post. Ese controlador valida
// el acceso del usuario al documento y luego hace `thread.sudo().message_post`
// — el sudo es lo que permite postear a usuarios portal, porque message_post
// internamente lee campos del proyecto (project_privacy_visibility) que el
// usuario portal no puede leer directamente (por eso un call_kw directo a
// message_post falla con AccessError). Usamos la sesión del PROPIO cliente,
// así Odoo valida su acceso y atribuye el mensaje a su partner.
export async function odooPortalPostMessage(sessionId, threadModel, threadId, body) {
  const response = await postJsonRpc(
    '/mail/message/post',
    {
